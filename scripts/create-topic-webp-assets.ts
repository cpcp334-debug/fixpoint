 ...
 * Deterministic SVG → WebP via sharp. No fake local job photos.
 */
import { mkdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import sharp from "sharp";

const TOPICS: Array<{ slug: string; label: string; hue: number; motif: string }> = [
  { slug: "plumbing", label: "Plumbing maintenance", hue: 205, motif: "faucet" },
  { slug: "ac", label: "Air conditioning service", hue: 195, motif: "ac" },
  { slug: "cleaning", label: "Professional cleaning", hue: 160, motif: "clean" },
  { slug: "painting", label: "Interior painting", hue: 25, motif: "paint" },
  { slug: "walls", label: "Wall maintenance", hue: 35, motif: "wall" },
  { slug: "electrical", label: "Electrical maintenance", hue: 45, motif: "socket" },
  { slug: "pool", label: "Pool cleaning", hue: 190, motif: "pool" },
  { slug: "sauna", label: "Sauna room care", hue: 15, motif: "sauna" },
  { slug: "appliance", label: "Appliance care", hue: 220, motif: "appliance" },
  { slug: "general", label: "Building maintenance", hue: 210, motif: "building" },
];

function svgFor(topic: (typeof TOPICS)[0]) {
...
async function main() {
  ...
  for (const topic of TOPICS) {
    const svg = Buffer.from(svgFor(topic));
    const webpTopic = join(topicsDir, `${topic.slug}.webp`);
    await sharp(svg).resize(1400, 900, { fit: "cover" }).webp({ quality: 82 }).toFile(webpTopic);
    created.push(`/media/topics/${topic.slug}.webp`);

    // Category aliases used by DIY categorySlug
